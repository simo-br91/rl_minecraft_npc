...
train_navigation.py
-------------------
Trains navigation with SHAPED rewards (the primary baseline).
Produces:
  python_rl/logs/nav_shaped_monitor.csv   — episode reward / length
  python_rl/logs/nav_shaped_success.csv   — per-episode success flag
  python_rl/checkpoints/nav_shaped_run1   — saved model
"""
...
def main() -> None:
    ...
    env = Monitor(env, filename=str(logs_dir / "nav_shaped_monitor.csv"))
    ...
    model.learn(total_timesteps=200_000, callback=success_cb)
    ...
    print("Monitor CSV: python_rl/logs/nav_shaped_monitor.csv")
    print("Success CSV: python_rl/logs/nav_shaped_success.csv")
